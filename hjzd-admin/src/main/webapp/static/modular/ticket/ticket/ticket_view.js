layui.use(["table", "element", "layedit", "form"], function () {
    ...
    form.on('submit(note)', function(data){
        ...
        $.ajax({
            url: Feng.ctxPath + '/ticket/ticket_note',
            type: 'post',
    ...
    table.render({
        ...
        height: 440,
        url: Feng.ctxPath + "/ticket/ticket_note_list",    // 数据接口
        page: true,    // 开启分页
        size: 'sm',
        method:'post',
        cols: [[    // 表头
    ...
    $(".btn-status").on("click", function () {
        ...
        if (status > 0) {
            Feng.confirm("是否修改工单状态", function () {
                $.ajax({
                    url: Feng.ctxPath + '/ticket/ticket_status',
                    type: 'post',
            ...
            Feng.confirm("是否删除工单", function () {
                $.ajax({
                    url: Feng.ctxPath + '/ticket/ticket_delete',
                    type: 'post',
                    data: {id: ticketId},
                    dataType: 'json',
                    success: function (res) {
