...
layui.use(["table", "element", "layedit", "form", "laydate"], function () {
    ...
    table.render({
        elem: '#ticketTable',
        height: 420,
        url: Feng.ctxPath + "/ticket_my/list",    // 数据接口
        page: true,    // 开启分页
        size: 'sm',
        method:'post',
        cols: [[    // 表头
            ...
            {title: '客户名', field: 'customName', align: 'center', valign: 'middle',width: 70},
    ...
    table.on('tool(ticketTable)', function(obj){    // 注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'edit'){
            ...
            var index = layer.open({
                ...
                fix: false, //不固定
                maxmin: true,
                content: Feng.ctxPath + '/ticket_my/ticket_edit?id=' + id
            ...
            var index = layer.open({
                ...
                fix: false, //不固定
                maxmin: true,
                content: Feng.ctxPath + '/ticket_my/ticket_view?id=' + id
            ...
            var operation = function(){
                var id = data['id'];    // 获取到当前行的id值
                var ajax = new $ax(Feng.ctxPath + "/ticket_my/ticket_delete", function (res) {
                    if (res.code == 200) {
                        Feng.success("删除成功!");
                    } else {
                        Feng.error("删除失败!");
    ...
    $("#create").click(function(){
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/ticket_my/ticket_add'
        });
        this.layerIndex = index;
        return false;
    });
    ...
    $("#import").click(function(){
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/ticket_my/ticket_import'
        });
        this.layerIndex = index;
        return false;
    });
