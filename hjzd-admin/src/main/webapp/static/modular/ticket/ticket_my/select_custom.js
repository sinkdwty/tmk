...
layui.use(["table", "element", "layedit", "form"], function () {
    ...
    table.render({
        elem: '#ticketTable',
        height: 440,
        url: Feng.ctxPath + "/ticket_my/custom_search",    // 数据接口
        page: true,    // 开启分页
        size: 'sm',
        method:'post',
        cols: [[    // 表头
            {title: '序号', type: 'numbers', align:'center', width: 60},
            {title: '客户姓名', toolbar:  '#bar'},
        ]],
        where: {
            deptId: deptId,
            order: 'desc'
    ...
    form.on('radio(select)', function(data){
        var customName = $(data.elem).attr("title");
        var customId = $(data.elem).val();

        window.parent.$("#customId").val(customId);
        window.parent.$("#customName").val(customName);
